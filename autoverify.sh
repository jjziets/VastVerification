...
tempOffers=($(./vast search offers  --limit 65535  "cuda_vers>=12.4 verified=any reliability>0.90 direct_port_count>3 pcie_bw>3 inet_down>10 inet_up>10 gpu_ram>7"  -o 'dlperf-'  | sed 's/|/ /'  | awk '{print $1,$11,$19,$20}'))
...
while (( ${#active_instance_id[@]} < 20 && ${#Offers[@]} > 0 )) || (( ${#active_instance_id[@]} > 0 )); do
	...
	for i in "${!active_instance_id[@]}"; do
		...
		if [ $exit_code -eq 2 ]; then
			...
			echo "Mark this Instance $instance_id for removal"
	                continue  # We've modified the array in the loop, so we break and start the loop anew
		#check if it has been waiting for more than 33min or if the instance has been running for 1m without any net response
